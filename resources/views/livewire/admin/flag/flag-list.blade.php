<x-admin.table>
    ...
    <x-slot name="tbody">
        @forelse($flags as $flag)
            <tr role="row" class="odd">
                ...
                <td>
                	@if($flag->image)
                		<img src="{{asset('storage/app/public/'.$flag->image) }}" style="width: 200px; height:60px;" alt=""  />
                	@else
			          <img src="{{asset('public/assets/images/inner_header.jpg') }}" alt="" style="width: 200px; height:60px;"/>
				    @endif
                </td>
                <td class="align-center"><span
                        class="kt-badge  kt-badge--{{ $flag->active == 1 ? 'success' : 'warning' }} kt-badge--inline kt-badge--pill cursor-pointer"
                        wire:click="changeStatusConfirm({{ $flag->id }})">{{ $flag->active == 1 ? 'Active' : 'Inactive' }}</span>
                </td>
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('flags.edit', ['flag' => $flag->id]) }}"><i
                            class="la la-edit"></i> Edit</a>
                    <button href="#" class="dropdown-item" wire:click="deleteAttempt({{ $flag->id }})"><i
    ...
    <x-slot name="showingEntries">
        Showing {{ $flags->firstitem() }} to {{ $flags->lastitem() }} of {{ $flags->total() }} entries
    </x-slot>
</x-admin.table>
