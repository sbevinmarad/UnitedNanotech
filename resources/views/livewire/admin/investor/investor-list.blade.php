<x-admin.table>
        ...
        @forelse($investors as $investor)
            <tr role="row" class="odd">
                ...
                <td class="align-center"><span
                        class="kt-badge  kt-badge--{{ $investor->active == 1 ? 'success' : 'warning' }} kt-badge--inline kt-badge--pill cursor-pointer"
                        wire:click="changeStatusConfirm({{ $investor->id }})">{{ $investor->active == 1 ? 'Active' : 'Inactive' }}</span>
                </td>
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('investors.edit', ['investor' => $investor->id]) }}"><i
                            class="la la-edit"></i> Edit</a>
                            <button href="#" class="dropdown-item" wire:click="deleteAttempt({{ $investor->id }})"><i
    ...
    <x-slot name="showingEntries">
        Showing {{ $investors->firstitem() }} to {{ $investors->lastitem() }} of {{ $investors->total() }} entries
    </x-slot>
</x-admin.table>
