<x-admin.table>
    ...
    <x-slot name="tbody">
        @forelse($provides as $provide)
            <tr role="row" class="odd">
                <td>{{ $provide->name }}</td>
                <td class="align-center"><span
                        class="kt-badge  kt-badge--{{ $provide->active == 1 ? 'success' : 'warning' }} kt-badge--inline kt-badge--pill cursor-pointer"
                        wire:click="changeStatusConfirm({{ $provide->id }})">{{ $provide->active == 1 ? 'Active' : 'Inactive' }}</span>
                </td>
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('provides.edit', ['provide' => $provide->id]) }}"><i
                            class="la la-edit"></i> Edit</a>
                    <button href="#" class="dropdown-item" wire:click="deleteAttempt({{ $provide->id }})"><i
    ...
    <x-slot name="showingEntries">
        Showing {{ $provides->firstitem() }} to {{ $provides->lastitem() }} of {{ $provides->total() }} entries
    </x-slot>
</x-admin.table>
