<x-admin.table>
    ...
    <x-slot name="tbody">
        @forelse($cms as $cmsDetails)
            <tr role="row" class="odd">
                <td>{{ $cmsDetails->title }}</td>
                <td class="align-center"><span
                        class="kt-badge  kt-badge--{{ $cmsDetails->active == 1 ? 'success' : 'warning' }} kt-badge--inline kt-badge--pill cursor-pointer"
                        wire:click="changeStatusConfirm({{ $cmsDetails->id }})">{{ $cmsDetails->active == 1 ? 'Active' : 'Inactive' }}</span>
                </td>
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('cms.edit', ['slug' => $cmsDetails->slug]) }}"><i
