        ...
        @forelse($services as $service)
            <tr role="row" class="odd">
                ...
                <td class="align-center"><span
                        class="kt-badge  kt-badge--{{ $service->active == 1 ? 'success' : 'warning' }} kt-badge--inline kt-badge--pill cursor-pointer"
                        wire:click="changeStatusConfirm({{ $service->id }})">{{ $service->active == 1 ? 'Active' : 'Inactive' }}</span>
                </td>
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('services.edit', ['service' => $service->id]) }}"><i
