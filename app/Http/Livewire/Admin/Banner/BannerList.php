...
class BannerList extends Component
{
    ...
    public function render()
    {
        ...
        if ($this->searchName)
            $bannerQuery->where('page_name', 'like', '%' . $this->searchName . '%');
        if ($this->searchStatus >= 0)
            $bannerQuery->where('active', $this->searchStatus);
    ...
    {
        $data = Banner::find($id['id']);
        if(isset($data->banner))
        {
            @unlink(storage_path('app/public/' .$data->banner));
        }
        $data->delete();
    ...
    {
        $this->showConfirmation("warning", 'Are you sure?', "You won't be able to recover this Banner!", 'Yes, delete!', 'deleteConfirm', ['id' => $id]); 
    }
    ...
    {
        $banner->fill(['active' => ($banner->active == 1) ? 0 : 1])->save();
