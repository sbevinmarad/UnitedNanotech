        ...
        @forelse($subscribers as $subscriber)
            <tr role="row" class="odd">
                ...
                <td class="align-center"><span
                        class="kt-badge  kt-badge--{{ $subscriber->active == 1 ? 'success' : 'warning' }} kt-badge--inline kt-badge--pill cursor-pointer"
                        wire:click="changeStatusConfirm({{ $subscriber->id }})">{{ $subscriber->active == 1 ? 'Active' : 'Inactive' }}</span>
                </td>
                <x-admin.td-action>
                    <button href="#" class="dropdown-item" wire:click="deleteAttempt({{ $subscriber->id }})"><i
