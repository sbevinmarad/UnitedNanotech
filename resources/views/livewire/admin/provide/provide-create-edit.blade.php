<x-admin.form-section submit="saveOrUpdate">
    <x-slot name="form">
    	<x-admin.form-group>
	        <x-admin.lable value="Provide Name" required />
	        <x-admin.input type="text" wire:model.defer="name" placeholder="Provide Name"  />
	        <x-admin.input-error for="name" />
	    </x-admin.form-group>

	    <x-admin.form-group>
	        <x-admin.lable value="Icon" required />
	        <x-admin.input type="text" wire:model.defer="icon" placeholder="Icon (transport)"  />
	    ...
	    <x-admin.form-group>
	        ...
	        @if($isEdit)
			    @if($provide->image)
		          <img src="{{asset('storage/app/public/'.$provide->image) }}" style="width: 100px; height:100px;" alt=""  />
		        @else
    ...
    <x-slot name="actions">
        ...
        <x-admin.link :href="route('provides.index')" color="secondary">Cancel</x-admin.link>
    </x-slot>
</x-form-section>